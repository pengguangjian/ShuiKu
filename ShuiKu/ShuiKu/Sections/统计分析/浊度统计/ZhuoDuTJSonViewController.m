...
@interface ZhuoDuTJSonViewController ()<AlterListViewDelegate,AddressListAlterViewDelegate>
@property (nonatomic , strong) UIButton *btselecttopitem;

@property (nonatomic , strong) ZhuoDuTJSonView *zview;
@property (nonatomic , assign) NSInteger type;
@end

@implementation ZhuoDuTJSonViewController
...
-(void)drawUI
{
    ...
    ZhuoDuTJSonView *view = [[ZhuoDuTJSonView alloc] initWithFrame:CGRectMake(0, 70, kMainScreenW, kMainScreenH-kTopHeight-120)];
    [self.view addSubview:view];
    [view mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.bottom.equalTo(self.view);
        make.top.equalTo(viewheader.mas_bottom).offset(10);
    }];
    view.strYValue = @"浊度NTU";
    view.strXValue = @"时间";
        ...
        if(state)
        {
            ...
            for(ZhuoDuFenXiModel *model in value)
            {
                ...
                {///进水
                    ...
                    [arrzuidajs addObject:model.max_TURB];
                    [arrzuixiaojs addObject:model.min_TURB];
                    
                }
                else if([model.s_type isEqualToString:@"14"])
                {///出水
                    [arrzuidacs addObject:model.max_TURB];
                    [arrzuixiaocs addObject:model.min_TURB];
                }
