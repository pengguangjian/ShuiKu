...
#import "WenDuTJListTableViewCell.h"
#import "TongJiFenXiDataController.h"
#import "WenDuFenXiModel.h"
#import "GetAreaModel.h"

@interface WenDuTJListViewController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource,AlterListViewDelegate,AddressListAlterViewDelegate>
...
@implementation WenDuTJListViewController
...
{
    static NSString *strcell = @"WenDuTJListTableViewCell";
    WenDuTJListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:strcell];
    if(!cell)
    {
        cell = [[WenDuTJListTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:strcell];
    }
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    NSArray *arrname = @[@"时间",@"最大进水℃",@"最小进水℃",@"最大出水℃",@"最小出水℃"];
        ...
        if(state)
        {
            ...
            for(WenDuFenXiModel *model in value)
            {
                ...
                {///进水
                    ...
                    [arrzuidajs insertObject:model.max_WT atIndex:0];
                    [arrzuixiaojs insertObject:model.min_WT atIndex:0];
                    
                }
                else if([model.s_type isEqualToString:@"14"])
                {///出水
                    [arrzuidacs insertObject:model.max_WT atIndex:0];
                    [arrzuixiaocs insertObject:model.min_WT atIndex:0];
