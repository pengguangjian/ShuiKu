    ...
    for (int i = 0; i < _lineChartDatas.count; i++) {
        ...
        if (chartData.showPointLabel) {//显示点
            ...
            for (NSValue *value in pointValueArray) {
                ...
                UITapGestureRecognizer *tabitem = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(lbvalueAction:)];
                [label addGestureRecognizer:tabitem];
...
///值点击
-(void)lbvalueAction:(UIGestureRecognizer *)gesture
{
    
    self.pointSelect((UILabel *)gesture.view);
